...
const DeleteTaskModal: React.FC<DeleteTaskModalProps> = ({ isOpen, onClose, onConfirm, type = 'task' }) => {
  const itemType = type === 'meeting' ? 'Meeting' : 'Task';
  ...
  return createPortal(
    ...
    >
      <div
        className="relative bg-white rounded-[10px] w-[371px] shadow-xl"
      ...
      >
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
            <mask id="mask0_1276_9511" style={{ maskType: 'luminance' }} maskUnits="userSpaceOnUse" x="0" y="0" width="22" height="22">
        ...
        <div className="p-[25px]">
          <div className="flex items-center gap-[5px] mb-5">
            ...
            <h3 className="text-[18px] font-semibold text-black">Delete {itemType}?</h3>
          </div>

          <p className="text-[16px] leading-normal font-normal text-black mb-[15px]">
            This action cannot be undone and all associated comments and files will be permanently removed.
          </p>

          {/* Buttons */}
          <div className="flex items-center justify-center gap-[10px] px-[26px]">
            <button
              type="button"
              onClick={() => {
                onConfirm();
              ...
              className="flex-1 border border-[#cacaca] rounded-[25px] px-[25px] py-[10px] text-[14px] leading-normal font-semibold text-[#d93025] hover:bg-[#F3F4F6] cursor-pointer transition"
            >
              Delete
            </button>
            <button
              type="button"
              onClick={onClose}
              className="flex-1 border border-[#cacaca] rounded-[25px] px-[25px] py-[10px] text-[14px] leading-normal font-semibold text-black hover:bg-[#F3F4F6] cursor-pointer transition"
            >
              Cancel
            </button>
          </div>
